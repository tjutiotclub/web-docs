...
在这一架构中，系统总线主要分为两类：一类是用于连接内核与高速资源的 AHB 总线，另一类是从 AHB 分支出来、专门用于连接外设的 APB 总线。为了兼顾不同外设对性能与功耗的需求，APB 总线又被划分为 APB1 和 APB2 两个子总线。其中，APB1 主要连接低速外设，而 APB2 则连接对时序要求较高的外设。

GPIO 在系统中属于由 CPU 直接控制的基础外设，其操作通常涉及频繁的电平翻转和快速响应（如外部中断、软件模拟通信等）。因此，相比于 I2C、CAN 等对速度要求较低的外设，GPIO 对总线访问速度具有更高要求。基于这一点，STM32 将 GPIO 安排在 APB2 总线上，使其能够工作在更高的时钟频率下，从而获得更快的响应速度和更低的访问延迟。

在 STM32F1 系列中，APB1 的最高工作频率通常为 36 MHz，而 APB2 可达到 72 MHz。GPIO 挂接在 APB2 上，可以充分利用更高的时钟频率，提高 I/O 翻转速度和系统实时性。
...
需要说明的是，虽然在库函数中也存在复用输入相关的配置形式，但在入门阶段的实际应用中较少涉及，因此本节不作展开介绍。
